...
def check_prerequisites(context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    container_name       = context.get("container") or "mycontainer"
    blob_name            = context.get("blob_name") or "sales.csv"

    blob_ls_name        = context.get("blob_ls_name") or os.getenv("ADF_BLOB_LINKED_SERVICE") or "AzureBlobStorageLinkedService"
    snowflake_ls_name   = context.get("snowflake_ls_name") or os.getenv("ADF_SNOWFLAKE_LINKED_SERVICE") or "Snowflake_LS"
    ...
    if not subscription_id:
        report.append(_missing("subscription_id", "Set AZURE_SUBSCRIPTION_ID or include 'subscription_id' in context"))
        return _finish(report)
    if not resource_group:
        report.append(_missing("resource_group", "Set AZURE_RESOURCE_GROUP or include 'resource_group' in context"))
        return _finish(report)
    if not factory_name:
    ...
    try:
        ...
        if any(getattr(s, "subscription_id", "") == subscription_id for s in subs):
            ...
            report.append(_missing("subscription", "az account set --subscription <id>", {"subscription_id": subscription_id}))
            return _finish(report)
    except Exception as e:
        report.append(_ok("subscription", {"subscription_id": subscription_id, "note": f"Could not enumerate ({e})"}))

    # providers check (coarse)
    try:
        ...
        report.append(_missing("providers",
    ...
    try:
        ...
        report.append(_missing("resource_group", f"az group create -n {resource_group} -l westeurope", {"error": str(e)}))
        return _finish(report)
    ...
    try:
        ...
        report.append(_missing("data_factory", f"az datafactory create -g {resource_group} -n {factory_name} -l westeurope", {"error": str(e)}))
        return _finish(report)
    ...
    if storage_account_name:
        try:
            ...
            report.append(_missing("storage_account", f"az storage account create -g {resource_group} -n {storage_account_name} -l westeurope --sku Standard_LRS --kind StorageV2",
                                   {"error": str(e)}))
        # data plane if key present
            ...
            try:
                ...
                if container_name in containers:
                    ...
                    if blob_name in blob_names:
                        ...
                        report.append(_missing("blob_exists",
                    ...
                    report.append(_missing("blob_container",
    ...
    try:
        ...
        if blob_ls_name in ls_names:
            ...
            report.append(_missing("blob_linked_service", f"Create Blob LS '{blob_ls_name}' (connection string SecureString or Managed Identity)."))
        if snowflake_ls_name in ls_names:
            report.append(_ok("snowflake_linked_service", {"name": snowflake_ls_name}))
        else:
            report.append(_missing("snowflake_linked_service", f"Create Snowflake LS '{snowflake_ls_name}' with a valid JDBC-like connection string."))
    except Exception as e:
    ...
    try:
        ...
        if source_dataset_name in ds_names:
            ...
            report.append(_missing("source_dataset", f"Create dataset '{source_dataset_name}' (Blob CSV) referencing '{blob_ls_name}'."))
        if sink_dataset_name in ds_names:
            report.append(_ok("sink_dataset", {"name": sink_dataset_name}))
        else:
            report.append(_missing("sink_dataset", f"Create dataset '{sink_dataset_name}' (Snowflake table) referencing '{snowflake_ls_name}'."))
    except Exception as e:
...
def auto_fix_prereqs(context: Dict[str, Any], initial_report: Dict[str, Any]) -> Tuple[bool, List[Dict[str, Any]]]:
    ...
    # Helper to see what's missing from initial_report
    missing_items = {i["item"] for i in initial_report.get("summary", {}).get("missing", [])}

    # 1) Blob Linked Service
    if "blob_linked_service" in missing_items and storage_account_name and storage_account_key:
    ...
    if "snowflake_linked_service" in missing_items and snowflake_conn_str:
        try:
            adf_client.linked_services.create_or_update(
                resource_group,
                factory_name,
    ...
    elif "snowflake_linked_service" in missing_items and not snowflake_conn_str:
    ...
    # 3) Source Dataset (Blob CSV)
    if "source_dataset" in missing_items and blob_ls_name and container_name and blob_name:
        try:
    ...
    # 4) Sink Dataset (Snowflake table)
    if "sink_dataset" in missing_items and snowflake_ls_name and snowflake_schema and snowflake_table:
        try:
            adf_client.datasets.create_or_update(
                resource_group,
                factory_name,
    ...
    elif "sink_dataset" in missing_items and not snowflake_ls_name:
        actions.append({"item": "sink_dataset", "action": "skipped", "status": "missing_input",
                        "error": "snowflake_ls_name not provided"})
